...
import { fullyColoredBlueHeart } from '@/components/UI/icons/blueHeart';
...
function AllDoctorScreen() {
    ...
    const { favoriteDoctors, addFavoriteDoctor, removeFavoriteDoctor } = useFavoriteDoctors();
    ...
    const toggleFavorite = (doctor: Doctor) => {
        if (favoriteDoctors.some((favDoctor) => favDoctor.id === doctor.id)) {
            removeFavoriteDoctor(doctor.id);
        } else {
            addFavoriteDoctor(doctor);
        }
    };
        ...
        <SafeAreaView style={[styles.container, containerStyle]}>
            ...
            <View>
                <View style={[styles.upper, containerStyle]}>
                    {!showSearch ? (
                        <HeaderComponent onSearchPressed={handleSearchPressed} headerText="Top Doctor" />
                    ) : (
                        <SearchComponent onSearchSubmit={handleSearchSubmit} filterAction={handleFilter} />
                    )}
                </View>
                <View style={[styles.categoryBtnView, containerStyle]}>
                    ...
                    >
                        {data.categories.map((category, index) => (
                            <Pressable
                                key={index}
                                style={[styles.categoryBtn, selectedCategory === category ? styles.firstCategoryBtn : {}]}
                            >
                                <Text
                                    style={[
